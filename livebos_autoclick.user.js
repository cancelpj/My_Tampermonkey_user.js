// ==UserScript==
// @name         livebos_autoclick
// @namespace    https://github.com/cancelpj/My_Tampermonkey_user.js
// @version      2016.10.22.1
// @description  LiveBOS 挂机。登录加载后找到页面下方导航栏中的“刷新”按钮（见脚本图标），然后定时模拟点击。有菜单可以设置刷新时间。
// @author       Peng Sir（QQ：9997452）
// @match        http://192.168.2.13:8008/UIProcessor?*
// @match        http://liveoa.fingu.com:8008/UIProcessor?*
...
var refreshTime = GM_getValue("sec",10)*1000*60;
var oldRefreshTime = refreshTime;

function setRefreshTime() {
    var sec1 = prompt('请输入刷新时间（分钟）：',GM_getValue("sec",10));
    if(sec1 !==null && sec1!=='' && parseInt(sec1)>0){
        GM_setValue("sec",Math.round(sec1));    // 四舍五入取整
        GM_log('设置LiveBOS挂机刷新时间（分钟）：'+ GM_getValue("sec",10));
    } else {
...
function run(){
    var timer = window.setInterval(function(){
        refreshTime = GM_getValue("sec",10)*1000*60;
        GM_log('LiveBOS挂机页面：'+ this.location.href);
        GM_log('LiveBOS挂机刷新时间（分钟）：'+ refreshTime/1000/60 + '    当前时间：'+ (new Date()).toLocaleTimeString());
        ...
        if(oldRefreshTime !== refreshTime){
            clearInterval(timer);
            GM_log('LiveBOS挂机刷新时间重新设置，重启定时器（分钟：'+ oldRefreshTime/1000/60 + ' ==> '+ refreshTime/1000/60 +  '）');
